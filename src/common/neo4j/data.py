...
async def init_schema():
    ...
    for schema in schemas:
        try:
            await run_neo4j_command(schema)
...
async def fill_data():    
    school1_id = await create_school_entity("test_univ", "Test University", "TU", 32.8801, 117.2340)
    school2_id = await create_school_entity("test_univ2", "Test University 2", "TU2", 30.4233, 100.4323)
    ...
    user_access_token_5, user_id_5 = await create_user_entity("Test User @ TU2", "testuser2", school2_id, False, False)
...
async def reset_db():
    ...
    await run_neo4j_command("""MATCH (n)
        DETACH DELETE n""")
    return 1

def init_neo4j():
    # Make sure neither both do_create_schema and do_reset_db are true, as they are asyncronous and can write over each other
