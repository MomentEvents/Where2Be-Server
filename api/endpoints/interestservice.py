...
async def update_event_interest(request: Request) -> JSONResponse:
    """
    Description: Updates and replaces all interest connections to {event_id}. User must be host (for user_access_token)
    ...
    try:
        assert all((event_id, user_access_token, interest_ids))
    except AssertionError:
        # Handle the error here
        print("Error")
        return Response(status_code=400, content="Parameter Missing")
    ...
    with get_connection() as session:
        # check if email exists
        result = session.run(
            """match (u:User{UserAccessToken: $user_access_token})-[:user_host]->(event:Event{EventID: $event_id}),
            (event)-[r:event_tag]->(i:Interest)
            delete r
            UNWIND $interest_ids as interest_id
            MATCH (tag:Interest {InterestID: interest_id})
            CREATE (tag)<-[et:event_tag]-(event)
            return et""",
            parameters={
                "event_id": event_id,
                "user_access_token": user_access_token,
